...
def distorted_inputs():
    """Construct distorted input for Face training using the Reader ops.
    ...
    images, labels = face_input.distorted_inputs(data_dir=data_dir,
...
def inputs(eval_data):
    """Construct input for Face evaluation using the Reader ops.
    ...
    if not FLAGS.data_dir:
        raise ValueError('Please supply a data_dir')
    data_dir = FLAGS.data_dir
    images, labels = face_input.inputs(eval_data=eval_data,
                                       data_dir=data_dir,
                                       batch_size=FLAGS.batch_size)
    if FLAGS.use_fp16:
        images = tf.cast(images, tf.float16)
...
def _add_loss_summaries(total_loss):
    """Add summaries for losses in Face model.
